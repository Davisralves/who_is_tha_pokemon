...
export default function GameBody({ pokemons, sortedPokemon, endGame, GameStatus }: Props) {
  ...
  console.log(pokemons);
    ...
    setTryedPokemon(pokemons.find(({name}) => name === inputValue) as IpokemonObject);
  ...
  const printTheDiferenceOfValues = (value1: number, value2: number): string => {
    switch(true) {
      case value1 > value2: return 'Higher'; 
      case value1 === value2: return 'Right'; 
      case value1 < value2: return 'Lower'; 
      default: return '';
    }
  }
  ...
  const pokemonTable = (
    <span>
      ...
      <p>| Type 1 | {checkIfIsEqual(type1, tryedPokemon.type1) ? 'Right' : 'Wrong' } |</p>
      <p>| Type 2 | {checkIfIsEqual(type2, tryedPokemon.type2 || 'None') ? 'Right' : 'Wrong' } |</p>
      <p>| Height | {printTheDiferenceOfValues(parseInt(height), parseInt(tryedPokemon.height)) } |</p>
      <p>| Weight | {printTheDiferenceOfValues(parseInt(weight), parseInt(tryedPokemon.weight)) } |</p>
    </span>
  );
	return (
		<section>
			...
			<form>
				...
				<button
					disabled={ !(pokemons.some(({name}) => name === inputValue ) && GameStatus === GameState.inProgress) }
