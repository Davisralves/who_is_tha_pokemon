...
const getPokemonsDetailsPromises = async (first151Pokemons: Ifirst151Pokemons) => {
  return await first151Pokemons.results.map(
    async ({ url }: Ipokemon) => {
...
const filterPokemonInfo = (pokemonsInfo: PromiseFulfilledResult<IpokemonDefaultObject>[]) => (
  pokemonsInfo.map((pokemonDetail: PromiseFulfilledResult<IpokemonDefaultObject>) => {
...
const PokemonModel = {
    ...
    const pokemonDetailsPromises = await getPokemonsDetailsPromises(first151Pokemons);
    const allPromises = await Promise.allSettled(pokemonDetailsPromises);
    const resolvedPromises = allPromises.filter((resolvedPromise) => resolvedPromise.status === "fulfilled") as PromiseFulfilledResult<IpokemonDefaultObject>[];
